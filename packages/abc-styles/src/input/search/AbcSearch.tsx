...
import styles from "./AbcSearch.module.css";
import AbcText from "../text/AbcText";
...
const AbcSearch: React.FC<Props> = ({
  threshold = 3,
  debounceTime = 300,
  placeholder = "Search...",
  id,
  ...
  const [lastSearch, setLastSearch] = useState(""); // 🔥 Almacena la última búsqueda

  useEffect(() => {
    if (searchTerm.length >= threshold && searchTerm !== lastSearch) {
      const timer = setTimeout(() => {
        onSearch?.(searchTerm);
        setLastSearch(searchTerm); // 🔥 Guarda la búsqueda para no repetirla
      }, debounceTime);

      return () => clearTimeout(timer);
    }
  }, [searchTerm, threshold, debounceTime, onSearch, lastSearch]);
  ...
  return (
    <div className={styles.searchContainer}>
      ...
      <AbcText id={id} label={placeholder} onChange={handleSearch} ref={inputRef} icon={icon()} />
    </div>
  );
};
